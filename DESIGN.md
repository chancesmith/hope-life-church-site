...
- **Accessible.** Text contrast: ink-on-sand and sand-on-evergreen both pass comfortably (12.9:1 and 9.2:1 on the original brand tokens; 17.8:1 and 8.7:1 on the current burgundy/gold values). The primary button (paper text on brass) does not, and never has — both the original `#B8863B` brass and the current burgundy-branch brass land around 3.2-3.3:1, below the 4.5:1 floor for normal-size text. See README for the follow-up. Keyboard focus rings are unstyled but not suppressed, so the browser default still shows. Alt text is present on both logo images.
